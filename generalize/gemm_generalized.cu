...
struct GemmImpl : public GemmRunner {
  ...
  RunResult run(int M, int N, int K, void *d_A, void *d_B, void *d_C,
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        ...
        GemmHandle::GemmKernel::kThreadCount, smem);
...
int main(int argc, char **argv) {
  ...
  size_t element_size_a = 4;
  size_t element_size_b = 4;
  size_t element_size_c = 4;
  ...
  size_t bytes_A = (size_t)max_m * max_k * element_size_a;
  size_t bytes_B = (size_t)max_k * max_n * element_size_b;
  size_t bytes_C = (size_t)max_m * max_n * element_size_c;
  ...
  csv << "M,N,K,Tile,Status,Time_ms,Cycles,TFLOPS,MFU,HBM_Eff,GridSize,"
         "ActiveBlocksPerSM,Waves,SplitK\n";
  for (const auto &case_res : summary) {
    for (const auto &run : case_res.all_runs) {
      csv << case_res.m << "," << case_res.n << "," << case_res.k << ","
          << run.name << "," << (int)run.status << "," << run.time_ms << ","
          << run.cycles << "," << run.tflops << "," << run.mfu << ","
          << run.hbm_eff << "," << run.grid_size << "," << run.active_blocks
